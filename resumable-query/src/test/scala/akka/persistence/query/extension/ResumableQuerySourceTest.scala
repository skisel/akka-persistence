...
class ResumableQuerySourceTest extends TestSpec {
  def withQueryFromOffset[A](queryName: String)(f: Flow[A, EventEnvelope, Future[Done]] => Unit): Unit = {
  ...
  def countEvents(pid: String): Future[Long] =
    journal.currentEventsByPersistenceId(pid, 0, Long.MaxValue).runWith(Sink.seq).map(_.size)

  def countEventsByTag(tag: String): Future[Long] =
    journal.currentEventsByTag(tag, 0L).runWith(Sink.seq).map(_.size)
  ...
  "single query" should "resume from the last offset" in {
    Source.repeat("foo").take(10)
      .zipWithIndex
      .map {
        case (text, index) => EventEnvelope(0L, "pid", index, s"$text-$index".tagged("t1"))
      }
      .grouped(100)
      .runWith(JournalWriter.sink("inmemory-journal")).futureValue

    eventually(countEventsByTag("t1").futureValue shouldBe 10)
