...
class QueryPerfSpec extends QueryTestSpec {
  def writeToJournal(numberOfEvents: Int): Unit = {
    ...
    println(s"Writing $numberOfEvents events to the journal took: ${end - start} ms")
  }
  ...
  def allEventsFromJournal: Seq[Any] = {
    ...
    val end = Platform.currentTime
    println(s"currrentEventsByPersistenceId for ${xs.size} events took: ${end - start} ms")
    xs
  }
  ...
  def eventsFromJournal(numberOfEvents: Int): Seq[Any] = {
    ...
    val xs = readJournal.eventsByPersistenceId("pid", 0, Long.MaxValue)
      ...
      .runWith(Sink.seq).futureValue
    val end = Platform.currentTime
    println(s"eventsByPersistenceId for ${xs.size} events took: ${end - start} ms")
    xs
  }
